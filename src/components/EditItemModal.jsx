...
function EditItemModal({ type, item, onClose }) {
  ...
  const getTitle = () => {
    switch (type) {
      ...
      case 'suppliers': return 'Edit Supplier';
      case 'locations': return 'Edit Location';
  ...
  const handleSubmit = (e) => {
    ...
    if (currentData.includes(itemName.trim()) && itemName.trim() !== item) {
      setError(`This ${type.slice(0, -1)} already exists`);
      return;
    }

    const actionType = `UPDATE_${type.toUpperCase().slice(0, -1)}`;
    
    dispatch({
      type: actionType,
      payload: { oldItem: item, newItem: itemName.trim() }
    });
